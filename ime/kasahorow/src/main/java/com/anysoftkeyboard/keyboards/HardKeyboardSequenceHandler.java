...
public class HardKeyboardSequenceHandler {
  ...
  public void addQwertyTranslation(String targetCharacters) {
    ...
    for (int qwertyIndex = 0; qwertyIndex < msQwerty.length; qwertyIndex++) {
      ...
      if (otherCharacter > 0) {
        ...
        this.addSequence(
            new int[] {KeyCodes.SHIFT, latinCharacter}, Character.toUpperCase(otherCharacter));
      }
    }
  }
  ...
  public int getCurrentCharacter(
    ...
    if (result == State.FULL_MATCH || result == State.PART_MATCH) {
      ...
      if (charactersToDelete > 0) inputHandler.deleteLastCharactersFromInput(charactersToDelete);
      return mappedChar;
    }
    return 0;
  }
